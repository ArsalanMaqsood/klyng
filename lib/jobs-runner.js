...
/*
 * aborts the job running on the current host
 * @param reason {String}: the reason for abortion
 */
function abort(reason) {

     // the nodes by now already recieved a signal to abort the job
     // handeling any monitor_socket error now (aka aborting the job)
     // is redundent and useless
     router.ignoreMonitorSocketErrors();

     router.routeToParent({type: 'job:aborted', reason: reason});

     // kill all local processes
     var locals = router.getLocalChannels();
     for(var rank in locals) {
         var proc = locals[rank];
         proc.kill();
     }

     // wait for a second to ensure that job:aborted is on the network if possible
     setTimeout(() => {
