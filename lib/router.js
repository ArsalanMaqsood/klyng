...
/*
 * sets a flag on monitor_socket to ignore any abrupt closure
 */
function ignoreMonitorSocketErrors() {
    if(!!monitor_socket) {
        monitor_socket.ignore_closure = true;
    }
}
 ...
 */
 function setRemoteChannel(id, sock) {
     ...
     if(!!waiting_queues['sock_' + id]) {
         waiting_queues['sock_' + id].forEach((msg) => sock.emit('KLYNG:MSG', msg));
         delete waiting_queues['sock_' + id];
     }
 }
    ...
    if(meta_routing_table['proc' + to] === 'local') {
        if(!!locals_routiung_channels['proc' + to]) {
            try {
                ...
                // swallow the error to prevent the beacon from failing
                // NOTE: This is a lousy quick fix, think of something better
            }
        }
        else {
        ...
        if(!!remote_routing_channels['sock_' + id]) {
            remote_routing_channels['sock_' + id].emit('KLYNG:MSG', msg);
        }
        else {
            if(!!waiting_queues['sock_' + id]) {
                waiting_queues['sock_' + id] = [];
            }
            waiting_queues['sock_' + id].push(msg);
        }
    }
}
...
module.exports = {
    ...
    ignoreMonitorSocketErrors: ignoreMonitorSocketErrors,
