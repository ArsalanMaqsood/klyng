    ...
    remoteIPC.serveNet('127.0.0.1', configurations.port, function() {
        ...
        remoteIPC.server.on('KLYNG:JOB', function(data, socket) {
            ...
            if(!!socket.klyng_secret && !!socket.klyng_authorized) {
                if(router.isClean()) {
                    var decrypted = utils.verify(data, socket.klyng_secret);
                    if(!!decrypted) {
                        ...
                        job.plan[socket.remoteAddress + ":" + job.plan.parent.port] = job.plan.parent;
                        delete job.plan.parent;

                        var hosts = Object.keys(job.plan).filter((host) => host !== "local");

                        // attempt to connect to all listed hosts
                        var connectionPromises = hosts.map((host) => {
                            var creds = host.split(":");
                            var promise = connectTo(creds[0], parseInt(creds[1]));

                            promise.then((connection) => {
                                router.setRemoteChannel(host, connection);
                                if(job.plan[host].isParent) {
