...
function authOver(connection, secret, passwd) {
    ...
    return new Promise(function(resolve, reject) {
        try {
            connection.emit('AUTH', cs.secure({data: passwd}, secret));
...
function sendJobOver(connection, secret, job, plan) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        try {
            connection.emit('KLYNG:JOB', cs.secure({data: job}, secret));
    ...
    remoteIPC.serveNet('0.0.0.0', configurations.port, function() {
        ...
        remoteIPC.server.on('KEY-EXT:PARAMS', function(data, socket) {
            try {
                ...
                if(!router.isClean()) {
                    throw new Error("Beacon is Busy.");
                }
                ...
                remoteIPC.server.emit(socket, 'KEY-EXT:PUBLIC', {
                    key: publicKey,
                    cipherWelcome: cs.secure("Hello from Beacon's TCP Server!", socket.klyng_secret)
                });
            }
            catch(error) {
        ...
        remoteIPC.server.on('AUTH', function(data, socket) {
            if(!!socket.klyng_secret) {
                ...
                try {
                    attemptPassword = cs.verify(data, socket.klyng_secret);
                }
        ...
        remoteIPC.server.on('KLYNG:JOB', function(data, socket) {
            ...
            if(!!socket.klyng_secret && !!socket.klyng_authorized) {
                if(router.isClean()) {
                    var decrypted;
                    try {
                        decrypted = cs.verify(data, socket.klyng_secret);
                    }
                    catch(error) { errorMessage = error.message; }

                    if(!!decrypted && !errorMessage) {

                        // this call to clear is to ensure that the waiting_queues are empty
                        // as some messages could have come after a past job was aborted and
                        // were stored in the waiting_queues
                        router.clear();
