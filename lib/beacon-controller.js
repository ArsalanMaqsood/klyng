...
function checkIfRunning() {
    ...
    return new Promise(function(resolve, reject) {
        ...
        // this flag indicates if the promise enclosing the connectTo call is fullfiled
        // and is used to determine if a connection faliure happened on the initial attempt
        // (aka before the promise is fullfiled) or during the process (after fullfillment)
        var promise_fullfiled = false;

        ipc.connectTo('klyng_beacon', function() {

            ipc.of.klyng_beacon.on('connect', function() {
                promise_fullfiled = true;
                resolve(true);
            });

            ipc.of.klyng_beacon.on('error', function(err) {
                if(ipc.of.klyng_beacon.retriesRemaining === 0) {
                    if(!promise_fullfiled) {
                        promise_fullfiled = true;
                        resolve(false);
                    }
                    else {
                        console.error('[Aborted]: Lost connection to local beacon');
                    }
                }
            });
        });
...
function start() {
    ...
    var beacon_daemon = cp.spawn('node', [__dirname + '/beacon.js'], {detached: true, stdio: 'ignore'});
    beacon_daemon.unref();
